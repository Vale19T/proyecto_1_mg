...
def consultar_orden_trabajo():
    try:
        ...
        if request.method == "POST" and 'txtBuscar' in request.form:
            ...
            cursor.execute("SELECT v.placa, s.fecha_entrada, s.fecha_salida, s.estado_vehiculo, s.estado_solicitud, ot.descripcion, ot.radio, ot.antena, ot.encendedor, ot.tapetes, ot.soat, ot.grua, ot.estado_llantas, ot.llave_pernos, ot.llanta_repuesto, ot.tapa_gasolina, ot.herramientas, ot.kit_carretera, ot.copas, ot.tarjeta_de_propiedad, ot.estado_proceso, ot.periodo_de_tiempo_ini, ot.periodo_de_tiempo_fin FROM vehiculo v LEFT JOIN solicitud_servicio s ON v.id_vehiculo = s.fk_id_vehiculo LEFT JOIN orden_trabajo ot ON s.id_solicitud = ot.fk_id_solicitud WHERE v.placa LIKE %s", (search_query,))
        else:
            cursor.execute("SELECT v.placa, s.fecha_entrada, s.fecha_salida, s.estado_vehiculo, s.estado_solicitud, ot.descripcion, ot.radio, ot.antena, ot.encendedor, ot.tapetes, ot.soat, ot.grua, ot.estado_llantas, ot.llave_pernos, ot.llanta_repuesto, ot.tapa_gasolina, ot.herramientas, ot.kit_carretera, ot.copas, ot.tarjeta_de_propiedad, ot.estado_proceso, ot.periodo_de_tiempo_ini, ot.periodo_de_tiempo_fin FROM vehiculo v LEFT JOIN solicitud_servicio s ON v.id_vehiculo = s.fk_id_vehiculo LEFT JOIN orden_trabajo ot ON s.id_solicitud = ot.fk_id_solicitud")
